...
class UnifiedMicrophoneNode(Node):
    ...
    def __init__(self):
        ...
        self.raw_rate = self.config_audio.get('rate', 48000)
        self.raw_channels = self.config_audio.get('channels', 2)
        self.raw_frame_size = self.config_audio.get('frame_size', 480)

        # DeepFilter audio parameters (fixed: 48kHz, 1ch, float32)
        self.df_topic = '/edie/audio/df_data'
        self.df_rate = 48000
        self.df_channels = 1
        self.df_frame_size = 480
        ...
        self.raw_src = PaRecordSource(
            rate=self.raw_rate,
            frames_size=self.raw_frame_size,
        ...
        self.df_src = PaRecordSource(
            rate=self.df_rate,
            frames_size=self.df_frame_size,
        ...
        self.get_logger().info(f'  Rate: {self.raw_rate}Hz, Channels: {self.raw_channels}')
        self.get_logger().info(f'  Frame size: {self.raw_frame_size}')
        self.get_logger().info(f'  Topic: {self.raw_topic}')
        self.get_logger().info(f'  Publish Rate: ~{self.raw_rate / self.raw_frame_size:.1f} Hz')
        self.get_logger().info('-' * 60)
        self.get_logger().info('[DeepFilter Audio - via parecord]')
        self.get_logger().info(f'  Source: {DF_DEVICE}')
        self.get_logger().info(f'  Rate: {self.df_rate}Hz, Channels: {self.df_channels}, Format: float32')
        self.get_logger().info(f'  Frame size: {self.df_frame_size}')
        self.get_logger().info(f'  Topic: {self.df_topic}')
        self.get_logger().info(f'  Publish Rate: ~{self.df_rate / self.df_frame_size:.1f} Hz')
        self.get_logger().info('=' * 60)
